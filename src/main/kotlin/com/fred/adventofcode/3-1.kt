...
fun sumPriorities(input: String) = input.lines()
    .asSequence()
    .map { splitInHalf(it) }
    .map { intersect(it) }
...
private fun intersect(it: Pair<String, String>) =
    it.first.toCharArray().intersect(it.second.toCharArray().asIterable().toSet())

private fun splitInHalf(line: String) =
    (line.length / 2).let { mid -> line.substring(0, mid) to line.substring(mid) }

fun main() {
    val input = """FzQrhQpJtJMFzlpplrTWjTnTTrjVsVvvTnTs
